...
### 创建模型
在app文件夹下创建model文件夹，所有的模型文件都在model文件夹下创建，然后将刚刚创建的迁移表中的内容复制过来。
```js
// app / model / user.js
...
## 查询
```ad-warning
当进行查询时，如果config.default文件中的config.sequelize的paranoid: true,则查询时会将时间戳的字段一起查询，如果没有三个时间戳字段，将会报错。
```
### 主键查询
...
            }
            // 参数验证异常
            if (status === 404 && error.message === "Validation Failed") {
                if (err.errors && Array.isArray(err.errors)) {
                    err = err.errors[0].err[0] ? err.errors[0].err[0] : err.errors[0].err[1];
                }
                ctx.body = {
                    msg: "fail",
                    data: err
...
## 验证
https://juejin.cn/post/7068572684229017613

使用方式：
https://github.com/D780/valparams/blob/master/doc/api.md
```
...
## crypto
https://www.cnblogs.com/xiuyun/p/13786037.html
